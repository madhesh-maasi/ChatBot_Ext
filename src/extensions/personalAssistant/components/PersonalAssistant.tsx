...
const PersonalAssistant: React.FC<IPersonalAssistantProps> = (props) => {
  ...
  const firstChatMessage: IChatMessage = {
    ...
    text: (
      <>
        Hi, I am your <b>personal assistant</b>. How can I help you today?
      </>
    ),
    date: null,
    avatar,
  ...
  const { getMyDetails, getMyEvents, getMyTasks, getMyDocuments } =
    useMicrosoftGraph(msGraphClientFactory);
  ...
  const showMessage = (
    ...
    replaceLastMessage: boolean = true
  ) => {
    if (replaceLastMessage) {
      ...
      setChatMessages((prevChatMessages) => {
        ...
        lastChatMessage.text = (
          <span dangerouslySetInnerHTML={{ __html: genericMessage }}>{}</span>
        );
  ...
  async function callFunction(functionName: string, functionArguments: any) {
    ...
    if (functionName === "getMyDetails") {
      ...
      functionResult = await getMyTasks(
        functionArguments.getIncompleteTasksOnly
      );
    } else if (functionName === "getMyDocumentManagement") {
    ...
    try {
      ...
      switch (response_finish_reason) {
        ...
        case "function_call": {
          ...
          console.log("function_arguments_json", function_arguments_json);

          switch (function_name) {
            case "getMyDetails":
            case "getMyEvents":
            case "getMyTasks":
            case "getMyDocumentManagement": {
              ...
              setOpenaiMessages((prevOpenaiMessages) => [
                ...prevOpenaiMessages,
                assistantMessage,
                functionMessage,
              ]);
